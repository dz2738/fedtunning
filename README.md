...
训练前的任务语义能否为新客户端提供合理的初始分组；

组内低维仿射 Prompt 子空间能否促进相关任务之间的迁移；

与共享方向正交的本地残差能否降低共享分量与个性化分量之间的干扰；

支持集适应和查询集反馈能否修正语义生成的初始坐标；

仅保留共享坐标路径的低维二阶梯度是否具有合理的性能—开销折中；

周期性 basis 维护能否吸收多个客户端反复出现的本地变化；

FedTaskPrompt 能否在提高平均性能的同时降低客户端负迁移率。
...
服务器根据任务语义为客户端分组，并生成初始坐标；

客户端在支持集上执行 $H$ 步局部适应；

客户端在查询集上计算查询损失和坐标反馈；

服务器在每个组内聚合反馈并更新坐标生成器；

满足维护条件时，服务器更新组中心或共享方向；

定期在客户端测试集上评价性能、负迁移和效率。
...
coordinate_second_order：只保留共享坐标路径上的 $H_{cc}v$，对应论文的低维二阶近似；可用
`second_order_steps` 截断时间方向的反向递推，并用 `hessian_damping` 配置
$H_{cc}+\lambda I$；
...
构造 support/query/validation/test 四分割；
...
support: 本地适应
query:   元训练反馈
validation: 周期性评测、早停和最优 checkpoint 选择
test:    最终评价

训练期间只评测 validation；测试集不得参与本地更新或服务器参数选择，并仅在训练结束后评测所选 checkpoint。
...
使用与客户端数据行区间隔离的公共代理配置初始化共享中心、方向和坐标生成器：

uv run python scripts/prepare_public_init.py public_data=prototype_proxy
...
10 轮中等规模验证、coordinate second-order 参数扫描以及 TensorBoard 客户端微调损失
可视化的完整命令见 `EXPERIMENT_GUIDE (1).md`。项目不保留仅用于封装 Hydra 参数的 shell
启动脚本。
...
实现 Prompt 合成和正交投影，并通过解析单元测试；

使用极小的合成 text-to-text 数据验证单客户端 support/query 更新；

用有限差分或完整 autograd 对照三种元梯度；

运行两个组、四个客户端的 smoke experiment；

加入 Local Prompt 和 FedAvg-Prompt 基线；

接入真实 Hugging Face 数据集；

增加冷启动、负迁移和 basis 维护实验；
...
pyproject.toml 与基础配置；

data/schema.py 和数据注册接口；

model/prompt_subspace.py 与对应测试；

model/backbone.py、soft_prompt.py；

model/task_encoder.py、coordinate_generator.py；

client/state.py、inner_loop.py、client.py；

server/group_state.py、grouping.py、aggregation.py；

server/basis_maintenance.py、server.py；

trainer/simulator.py、评价与 checkpoint；

基线、运行脚本和端到端 smoke test。
